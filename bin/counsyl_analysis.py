...
def sample_type_proportions( samptypedf ) :
    ...
    for sample_type, subdata in samptypedf.groupby("sample_type") :
        subdata["proportion"] = subdata["Count"].apply(lambda x : x /
...
# Generate a bar plot showing the propotions of samples with different QC statuses
# Across sample types
def partb_plot_qc_proportions( sampleqccnts, figdir ) :
    ...
    p = (ggplot2.ggplot(r_dataframe) +
         ggplot2.aes_string(x="qcstatus",y="proportion",fill="factor(sample_type)" ) +
...
# partc_plot_metrics
# Generate a box plot to show distributions of sample values across different
# QC metrics and comparing sample types
def partc_plot_metrics( metricsdf, figdir ) :
    r_dataframe = com.convert_to_r_dataframe( metricsdf )

    p = (ggplot2.ggplot(r_dataframe) +
         ggplot2.aes_string(x="sample_type",y="value",fill="factor(qcstatus)" ) +
...
# Generate a candlestick like plot showing the mean and standard devations for
# QC metrics over subsequent batches
def partd_plot_batch_stats( batchstats, figdir ) :
    ...
    p = (ggplot2.ggplot(r_dataframe) +
         ...
         ggplot2.geom_errorbar(ggplot2.aes_string(ymin="sd_low", ymax="sd_high"),
                               size=1, width=.1) +
         ggplot2.scale_y_continuous("QC Metric Value") +
...
if __name__ == "__main__" :
    ...
    assert os.path.exists( figure_dir ), "Error: defined target directory not found"
    ...
    # What percentage of samples passed sequencing coverage but failed due to CNV calling?
    ################################################################################ 
    ...
    # Does "sample_type" # relate to passing coverage but failing CNV calling QC?
    ################################################################################ 
    print "Does Sample type relate to failing sequencing coverage QC?"
    sampleqccnts = DataFrame({"Count":sampledf.groupby(["sample_type","coverage"]).size()
                             }).reset_index()
    sampleqccnts = sample_type_proportions( sampleqccnts )
    print sampleqccnts

    print "Does Sample type relate to passing coverage but failing CNV calling?"
    sampleqccnts = DataFrame({"Count":sampledf.groupby(["sample_type","coverage","CNV_calling"]
                                                           ).size() }).reset_index()
    sampleqccnts = sample_type_proportions( sampleqccnts )
    sampleqccnts["qcstatus"] = sampleqccnts["coverage"] + ":" + sampleqccnts["CNV_calling"]
    print sampleqccnts

    # Generate a bar plot showing the differences in distribution between QC statuses
    partb_plot_qc_proportions( sampleqccnts, figure_dir )

    ################################################################################ 
    # C)
    # Which in-process QC metrics are indicative of failing sequencing due to low coverage?
    ...
    batchstats = DataFrame( batchstats, columns=["Batch","metric","mean","std"])
    ...
    batchstats["nbatch"] = batchstats["Batch"].apply(lambda x: int(x[x.find("_")+1:]))
